...
Windows App SDK 1.8では、特定のXAML要素の範囲にSystemBackdropを適用できる `SystemBackdropElement` を利用候補とする。技術スパイクで角丸、クリップ、Translation、透明Hostとの組み合わせを確認する。
...
1. `SystemBackdropElement` + `DesktopAcrylicBackdrop`
2. Surface単位のDesktop Acrylicが要件を満たさない場合、Window全体の `DesktopAcrylicBackdrop`
...
3. `SystemBackdropElement` のDesktop AcrylicがSurfaceと一緒に移動する。
