...
更新日: 2026-07-22  
対象: Windows_SC `0.5.4-dev` / Windows App SDK 1.8 / x64
...
- 手動表示はウィンドウをアクティブ化し、167ms、24 effective pixelの進入を使う。前面化を即時確認し、失敗時は50ms間隔、最大5回だけ再要求する。
...
- Explorerの`StartButton`もスタート本体のフォーカスも取得できないOS状態では、複数`SearchHost`候補のクリック元モニターを確定できない可能性がある。
