...
| `UiAutomationStartMenuInspector.cs` | 専用STAスレッド、フォーカスイベント、Win32優先スキャン、UI Automation補助検出、Snapshot更新。 |
...
### 4.1 第1段: Win32ウィンドウ検出

`StartMenuWindowInspector.TryGetStartMenuBounds` を最初に実行する。
...
この経路は深いUIツリーを読まず、通常の起動確認とスタートボタンクリックの検出に使う。

### 4.2 第2段: UI Automation補助検出

Win32で取得できない場合のみ、フォーカス要素を確認する。
...
`_startReopenRequestedDuringExit` は、退出中に新しいWindowsキー単体入力を受けた場合だけ `true` になる。Snapshotが `Visible` という理由だけでは反転しない。
...
| UI Automationイベント登録とスキャン | 専用STAスレッド |
